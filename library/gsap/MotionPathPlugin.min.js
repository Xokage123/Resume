...
!(function (t, e) {
	...
	function sliceRawPath(t, e, n) {
		...
		if (p || r) {
			for (
				i = w < b || (R && M < L) || (T && m.t < d.t),
					y(a, b, L, d.t) &&
						(b++,
						i ||
							(w++,
							T ? ((m.t = (m.t - d.t) / (1 - d.t)), (M = 0)) : R && (M -= L))),
					1 - (n - e) < 1e-5
						? (w = b - 1)
						: !m.t && w
						? w--
			...
			for (
				s = subdivideSegment(P, L, T ? d.t / m.t : d.t),
					R && (l += s),
					v.splice(M + l + 2),
					(s || L) && P.splice(0, L + s),
					h = a.length;
	...
	function measureSegment(t, e, n) {
		...
		for (T.length = S.length = 0, a = e + 2; a < b; a += 6) {
			if (
				...
				(f = t[a + 1] - L),
				(l = h = c = p = 0),
				Q(o) < 1e-5 && Q(u) < 1e-5 && Q(s) + Q(f) < 1e-5)
			)
		...
		if (T.length && N)
			for (
				t.totalLength = P = T[T.length - 1] || 0,
					t.minLength = N,
					x = v = 0,
					r = 0;
				r < P;
		...
		for (e = 0; e < y; e++)
			if (
				...
				(u = x),
					(g = P),
					('C' !== c && 'S' !== c) ||
						((u += x - s[s.length - 4]), (g += P - s[s.length - 3])),
					i || (x = P = 0),
	...
	function pointsToSegment(t, e, n) {
		...
		var r,
			...
			M = y - x,
			L = w - P,
			R = Math.abs(t[v] - x) < 0.001 && Math.abs(t[v + 1] - P) < 0.001;
		for (
			...
			(r = x),
				...
				(x === y && P === w) ||
					...
					(d = (M = y - x) * M + (L = w - P) * L),
					(m = (h = y - r) * h + (g = w - a) * g),
					(c = ((o = Math.acos((p + d - m) / W(4 * p * d))) / Math.PI) * e),
					(f = W(p) * c),
					(c *= W(d)),
					(x === r && P === a) ||
						(n < o
							...
							: ((i = S(u, l)),
							  b.push(C(x - H(i) * f), C(P - U(i) * f)),
							  (i = S(L, M)),
							  b.push(C(x), C(P), C(x + H(i) * c), C(P + U(i) * c)))));
		return (
			x !== y || P !== w || b.length < 4
				? b.push(C(y), C(w), C(y), C(w))
				: (b.length -= 2),
			R && (b.splice(0, 6), (b.length = b.length - 6)),
			b
		);
	}
		...
		if ((u.length || u.push(X(t, 1), X(t, 2), X(t, 3)), (n = l ? b : f), l))
			...
			if (((o = i = 0), I))
				for (
					...
					4 < (m.getComputedStyle(a)[E] + '').length &&
	...
	function getGlobalMatrix(t, e, n, r) {
		...
		var a = (function _forceNonZeroScale(t) {
			...
			g =
				...
				(function _isFixed(t) {
					return (
						'fixed' === m.getComputedStyle(t).position ||
	...
	function la(t, e, n, r, a, o, i, s, l) {
		return (
			(e =
				'cubic' === i.type
					? [e]
					: (e.unshift(ja(n, r, s), a ? ja(n, a, l) : 0),
					  i.relative && ka(e),
				...
				? (i &&
						('self' === i || (r = J(i)[0] || e) === e
							...
							: (u && !1 !== u[2]
									...
									: (u = [ja(e, 'xPercent') / -100, ja(e, 'yPercent') / -100]),
							  (o = (a = pa(e, r, u, 'auto')).apply({ x: g, y: f })),
							  transformRawPath(
									t,
									a.a,
									a.b,
			...
			init: function init(t, e) {
				...
				if (
					...
					g))
						r !== h &&
							r !== u &&
							a.push(la(this, ia([], o, r, 2), t, r, 0, f, e, q(o[0][r])));
				}
			},
